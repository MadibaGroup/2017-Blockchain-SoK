...
Tthis provides the provenance and resilience necessary for such transactions.
...
While access control systems are nominally distributed---it might require several written signatures to get permissions on a system---in practice it is highly centralized---a single administrator can unilaterally grant nearly any access to the network.
...
One key challenge when sharing data between multiple organizations is that no organizations wants to get up ownership and control of their own data.
...
Identities along with cryptographic attestations about properties for those identities (e.g., over 18, has a driver's license) can be written to the Blockchain.
These identities and attestations can then be used by other systems to support their access control policies.
Importantly, this identity information comes with full provenance. This could be useful in determine suspicious activity (e.g., having an age that jumps around).
...
A Blockchain system's append-only ledger can be used to stored documents, including the history of changes to these documents.
...
This auditability is especially important as IoT devices are notoriously untrustworthy, and the ability to latter audit and analyze their actions is invaluable.
...
Much like the insurance use cases, regulations can be written to a Blockchain and as off-chain oracles report on real-world violations of those regulations.
...
Blockchain technology can facilitate the interoperation of multtiples systems.
...
This is a poor use cases because it does not rely on shared governance, consensus, or provenance.
This use case can more easily be accomplished with a standard program and a database.

\subsubsection{Gambling}
A Blockchain system could record bets, execute games, deduct funds in the case of lose, and payout in the case of a win.
The audibility of such a system could confidence to the players of the system.
Additionally, the system could have its own cryptocurrency, providing a one-stop shop for gambling.
...
If the gambling is regarding the outcome of real-world events, then the system has to rely on off-chain oracles to settle bets. This could slow down the system and also introduce a single point of failure.

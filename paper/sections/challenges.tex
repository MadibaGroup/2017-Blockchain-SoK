...
\anote{This is just a list for now, I will write up the text for this this week.}
\begin{itemize}
...
As blockchain-based systems such as Bitcoin have grown, the increased number of transactions and users have raised questions about the scalability, cost, and performance of such systems.  Many of the documents we coded point out that blockchain is \emph{not a high performance system} and that its decentralized governance and operation results in an inefficient use of resources.  In particular, most of today's permissionless blockchains are based on proof-of-work style consensus in which consensus is achieved by having miners solve computationally difficult puzzles to extend the blockchain.  This has led to the proliferation of power-hungry specialized hardware devices dedicated to solving these computational puzzles.  According to one estimate\cite{Digiconomist}, as of April 2018 the energy consumed by Bitcoin miners was equivalent to the power usage of almost 5.5 million US households, and all signs indicate that it will continue to grow.
...
Due to its decentralized nature, blockchain is also potentially vulnerable to a number of security threats.  First, coordinated attacks by a majority (and often even a large minority) of the miners can reorder, remove, and change transactions from the blockchain thus destroying all guarantees that the blockchain provides.  Thus, it is critical that blockchain-based solutions provide the proper incentives to keep the miners honest.  In particular, it is critical to design incentives such that the Nash equlibrium behavior for selfish miners is to honestly maintain the state of the blockchain rather than to destroy it through forming such coalitions.

Additionally, blockchain solutions are potentially vulnerable to traditional network attacks such as denial of service.  Such attacks can aim to bring down a fraction of the mining power, or to partition the network of miners to try to achieve an inconsistent state.  Thus, blockchain solutions must be able to recover to a valid state even after such a denial of service attack.
\anote{Not sure what security of infrastructure -- is this key management?, and subversion of software security measures means.}

\subsection{Privacy and anopnymity on the blockchain}
Another major challenge for blockchain applications is how to protect the privacy of the users and data stored on the blockchain.  By design, all data on the blockchain is public to enable verification by all miners.  But, this means that any sensitive data placed (in the clear) on the blockchain is inherently non-private.  Cryptographic approaches such as encryption and zero-knowledge proofs are needed\anote{Add citations} to protect the privacy of such data.

Moreover, if one wishes to hide the parties participating in a transaction recorded on the blockchain, it is not enough to hide the transaction information.  It is also necessary to protect the anonymity of these parties by hiding the fact that they connected to the blockchain in the first place.  While most blockchains provide a notion of ``pseudonymity'', users are identified by their cryptographic keys instead of by their names or social security numbers, it has been shown that this is not enough to provide true anonymity as attacks that correlate transactions by the same pseudonyms together with access histories can deanonymize blockchain transactions\anote{Add citations}.

\subsection{Usability of blockchain tools}
Yet another important challenge that arises when building applications on top of blockchain technology is the lack of development and blockchain analysis tools.  Currently, there are no universal tools available for creating transactions and smart contracts for many of the existing blockchains, this is even more true if we want tools that can be used across multiple blockchains.  As mentioned earlier, the lack of smart contract development and testing tools makes the chance of critical bugs higher.  Additionally, there is a lack of testing support and infrastructure to test out one's applications (for correctness, performance, and security) before deploying it on the blockchain.  Additionally, many of the blockchains are currently geared towards expert users and lack the user experience focused tools needed to allow for easier use by non-experts.

Additionally, a much touted capability of blockchain is the auditability of all transactions on the blockchain.  However, there is no external API to allow an auditor access to a blockchain on which she is not a miner.  Thus, auditors are forced to run full mining nodes just to audit the blockchain. 
...
Finally, before a company chooses to deploy a blockchain-based application it should carefully consider whether blockchain is the right tool for the job.  Due to the hype around this technology many solutions have turned to blockchain when other more established technologies such as traditional databases would do the job just fine.  For example, in situations when a centralized repository is easily available, it is likely to be preferable to a blockchain.  In fact there are relatively few use-cases where blockchain is demonstratively the ``right'' solution.  While we believe that more such use-cases will be identified, when evaluating a new use-case, it is important to make sure the blockchain is really needed and that a lower-overhead solution will not suffice.

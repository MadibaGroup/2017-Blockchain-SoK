...
As mentioned earlier, the lack of smart contract development and testing tools increases the chance of critical bugs.  Additionally, there is a lack of testing support and infrastructure to test out one's applications (for correctness, performance, and security) before deploying them. Many blockchains are currently geared towards expert users and lack the user experience-focused tools needed to allow for easier use by non-experts.

Additionally, a much touted capability of blockchain is the auditability of all transactions.  However, there is no external API to allow an auditor access to a blockchain on which she is not a miner.  Thus, auditors are forced to run full mining nodes just to audit the blockchain. 


% Jeremy: I see the below as usability challenges and can be merged there.
